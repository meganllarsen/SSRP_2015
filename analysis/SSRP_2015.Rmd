...
for (name in file.names){
  ...
  res.use.data <- as.matrix(read.table(paste(file.path, name, sep='')))
  i1.vals <- as.numeric(res.use.data[,1:4])
  i1.var <- var(i1.vals)
  ...
  i2.vals <- as.numeric(res.use.data[,5:8])
  i2.var <- var(i2.vals)
  ...
  i3.vals <- as.numeric(res.use.data[,9:12])
  i3.var <- var(i3.vals)
  results <- cutoff(i1.vals)
  i3.num <- results[1] 
  i3.mean <- results[2]  
  ...
  file.name.info <- c(file.name.info, i1.mean, i1.sd, i1.num)
  file.name.info <- c(file.name.info, i2.mean, i2.sd, i2.num)
  file.name.info <- c(file.name.info, i3.mean, i3.sd, i3.num)
  file.name.data <- rbind(file.name.data, file.name.info)
  }

dat.frame <- data.frame(matrix(unlist(file.name.data),
                      nrow=28, byrow=F),stringsAsFactors=TRUE)
dat.frame<- setNames(dat.frame, c('data', 'strain', 'Ancestor_or_Derived',
                      'plate', 'plate.replicate', 'duration',
                      'i1.mean', 'i1.sd', 'i1.num',
                      'i2.mean', 'i2.sd', 'i2.num',
                      'i3.mean', 'i3.sd', 'i3.num'))
...
for (name in file.names){
  ## Load Data
  a <- read.table(paste(file.path, name, sep=''))
