...
for (name in file.names){
  ...
  i1.var <- var(i1.vals)
  list[i1.num, i1.mean, i1.sd] <- cutoff(i1.vals)
    
  i2.vals <- as.numeric(res.use.data[,5:8])
  i2.var <- var(i2.vals)
  list[i2.num, i2.mean, i2.sd] <- cutoff(i1.vals)
  
  i3.vals <- as.numeric(res.use.data[,9:12])
  i3.var <- var(i3.vals)
  list[i3.num, i3.mean, i3.sd] <- cutoff(i1.vals)
  
  file.name.info <- c(file.name.info, i1.mean, i1.num)
...
dat.frame<- setNames(dat.frame, c('data', 'strain', 'AorD',
                      'plate', 'plate.replicate', 'duration', 'i1', 'i2', 'i3'))
